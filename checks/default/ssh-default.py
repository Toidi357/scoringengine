...
def run(host, metadata):
    """
    This check attempts an ssh connection to a host with given credentials, and simply runs "whoami"
    ...
    try:
        ...
        with Connection(**conn_params) as c:
            # We run a simple, lightweight command to verify the shell is interactive
            c.run('whoami', hide=True, timeout=7)
            return True
